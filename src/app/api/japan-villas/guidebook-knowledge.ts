...
export const PROPERTY_KNOWLEDGE: { [key: string]: PropertyKnowledge } = {
  "LAKE HOUSE 野尻湖": {
    ...
    wifi: {
      networks: [
        { name: "The Lake House - 2G", password: process.env.LAKEHOUSE_WIFI_PASSWORD || "[REDACTED]" },
        { name: "The Lake House - 5G", password: process.env.LAKEHOUSE_WIFI_PASSWORD || "[REDACTED]" }
      ]
    },
    keyFeatures: [
  ...
  "The Lake Side INN": {
    ...
    wifi: {
      networks: [
        { name: "Cabin 1", password: process.env.LAKESIDE_CABIN1_WIFI || "[REDACTED]" },
        { name: "Cabin 2", password: process.env.LAKESIDE_CABIN2_WIFI || "[REDACTED]" },
        { name: "Cabin 3", password: process.env.LAKESIDE_CABIN3_WIFI || "[REDACTED]" },
        { name: "Cabin 4", password: process.env.LAKESIDE_CABIN4_WIFI || "[REDACTED]" }
      ]
    },
    keyFeatures: [
    ...
    commonQuestions: {
      wifi: {
        answer: `The Lake Side INN has separate WiFi networks for each cabin:\n\n📶 **Cabin 1:** Network name varies, Password: **${process.env.LAKESIDE_CABIN1_WIFI || "[REDACTED]"}**\n📶 **Cabin 2:** Network name varies, Password: **${process.env.LAKESIDE_CABIN2_WIFI || "[REDACTED]"}**\n📶 **Cabin 3:** Network name varies, Password: **${process.env.LAKESIDE_CABIN3_WIFI || "[REDACTED]"}**\n📶 **Cabin 4:** Network name varies, Password: **${process.env.LAKESIDE_CABIN4_WIFI || "[REDACTED]"}**\n\n🏠 Please check which cabin you're staying in and use the corresponding password. Each cabin also has a WiFi QR code for instant connection!`,
  ...
  "MOUNTAIN VILLA ニセコ": {
    ...
    wifi: {
      networks: [
        { name: "mvniseko", password: process.env.MOUNTAINVILLA_WIFI_PASSWORD || "[REDACTED]" }
      ]
    },
    keyFeatures: [
